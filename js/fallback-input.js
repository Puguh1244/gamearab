...
function enableKeyboardFallback() {
  ...
  document.addEventListener('mousedown', () => {
    if (fallbackInputActive && GameState.started) HandData.leftPinching = true;
  });

  document.addEventListener('mouseup', () => {
    if (fallbackInputActive) HandData.leftPinching = false;
  });
...
function stripPdfUnsupportedSymbols(value) {
  return String(value ?? '')
...
function getCleanPdfScore() {
  const rawScore = stripPdfUnsupportedSymbols(
    getReportText('go-score', String(GameState.score || 0))
  );

  const numberMatch = rawScore.match(/\d+/g);
  const finalScore = numberMatch
    ? numberMatch[numberMatch.length - 1]
    : String(GameState.score || 0);

  if (/semua level selesai/i.test(rawScore)) {
    return `Semua Level Selesai! Skor Akhir: ${finalScore}`;
  }

  return `Skor Akhir: ${finalScore}`;
}
  ...
  root.innerHTML = `
    ...
    <div class="pdf-header">
      ...
      <p class="pdf-subtitle">
        Hasil permainan hijaiyah berbasis gestur tangan, MediaPipe, dan AI Recommendation.
        Dibuat otomatis pada ${pdfEscape(now)}.
      </p>
    </div>
    ...
    <div class="pdf-meta">
      ${metrics.map(([label, value]) => `
        <div class="pdf-metric">
          <span>${pdfEscape(label)}</span>
          <strong>${pdfEscape(stripPdfUnsupportedSymbols(value))}</strong>
        </div>
      `).join('')}
    </div>

    <div class="pdf-alert">
      Materi perlu penguatan: <span dir="rtl">${pdfEscape(stripPdfUnsupportedSymbols(weakLetters))}</span>
    </div>

    ${sections.map(([title, body]) => `
      <section class="pdf-section">
        <h3>${pdfEscape(title)}</h3>
        <p>${pdfEscape(stripPdfUnsupportedSymbols(body))}</p>
      </section>
    `).join('')}
...
function getSafePdfText(value) {
  return stripPdfUnsupportedSymbols(value) || '-';
}
...
function generateReportWithJsPdf(payload) {
  ...
  doc.setFontSize(9);
  doc.text(`Dibuat otomatis pada ${getSafePdfText(payload.generatedAt)}`, margin + 7, y + 28);
  ...
  metrics.forEach(([label, value], i) => {
    ...
    const safeValue = getSafePdfText(value);
    const wrappedValue = doc.splitTextToSize(safeValue, colW - 8).slice(0, 2);
    doc.text(wrappedValue, x + 4, boxY + 11);
  });
  ...
  payload.sections.forEach(([title, body]) => {
    ...
    doc.text(getSafePdfText(title), margin + 4, y + 8);
